...
import { Printer, Download, ArrowLeft, CreditCard, Wallet, CheckCircle, ShoppingBag, X, Minus, Plus, User, MapPin, Phone, History, Trash2, ShoppingCart } from 'lucide-react';
...
const Billing = () => {
  ...
  const { items: initialItems = [], total = 0, paymentMethod: initialPayment = 'cash' } = billData;
  ...
  const handleCashPayment = () => {
    ...
    if (!cash || cash < grandTotal) {
      addToast(`Please enter amount greater than or equal to ₹${grandTotal.toFixed(2)}`, 'error');
      return;
    }
  ...
  const paymentOptions = [
    { id: 'cash', label: 'Hand Cash', icon: <Wallet size={20} />, color: 'green' },
    { id: 'online', label: 'Online Payment', icon: <CreditCard size={20} />, color: 'purple' },
    { id: 'barrow', label: 'Barrow / Credit', icon: <User size={20} />, color: 'orange' }
  ];
      ...
      <div className="p-6 bg-gray-100 min-h-screen">
        <div className="flex flex-wrap justify-between items-center gap-4 mb-6">
          ...
          <div className="flex gap-3">
            ...
            >
              <History size={20} /> {showHistory ? 'Hide' : 'Show'} History ({allBills.length})
            </button>
          </div>
        </div>
        ...
        <div className="mt-6 max-w-4xl mx-auto">
          <div className="bg-white rounded-xl shadow-lg overflow-hidden">
            ...
            <div className="p-4">
              {allBills.length === 0 ? (
                <p className="text-gray-500 text-center py-4">No bills found. Complete a payment to see history here.</p>
              ) : (
                  ...
                  {allBills.map((bill, index) => (
                    <div 
                      ...
                      className={`bg-gray-50 rounded-lg p-3 hover:bg-gray-100 transition border-l-4 ${
                        bill.status === 'Paid' ? 'border-green-500' : 'border-orange-500'
                      }`}
                    >
                      <div className="flex justify-between items-start">
                        <div>
                          <p className="font-bold text-gray-800 text-sm">{bill.id}</p>
                          <p className="text-xs text-gray-500">{formatDate(bill.date)}</p>
                          <p className="text-xs text-gray-600 mt-1">
                            <span className="font-medium">Customer:</span> {bill.customer?.name || 'N/A'}
                          </p>
                          <p className="text-xs text-gray-600">
                            <span className="font-medium">Items:</span> {bill.items?.length || 0}
                          </p>
                        </div>
                        <div className="text-right">
                          <p className="text-lg font-bold text-blue-600">₹{bill.grandTotal?.toFixed(2)}</p>
      ...
      <div className="bg-white rounded-xl shadow-lg max-w-4xl mx-auto overflow-hidden">
        ...
        <div className="p-6 border-b">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <p className="font-medium">{date.toLocaleDateString('en-IN', { 
                day: '2-digit', 
                month: 'short', 
                year: 'numeric' 
              })}</p>
              <p className="text-sm text-gray-500 mt-2">Time</p>
              <p className="font-medium">{date.toLocaleTimeString('en-IN', { 
                hour: '2-digit', 
                minute: '2-digit' 
              })}</p>
            </div>
            <div className="text-right">
              ...
              <div className="flex items-center justify-end gap-2 font-medium text-blue-600">
                {paymentMethod === 'cash' ? <Wallet size={20} /> : 
                 paymentMethod === 'online' ? <CreditCard size={20} /> : 
                 <User size={20} />}
                <span>{paymentMethod === 'cash' ? 'Hand Cash' : 
        ...
        <div className="p-6">
              ...
              <tbody>
                {items.map((item, index) => (
                  <tr key={item._id || item.id || index} className="border-b border-gray-100">
                    ...
                    <td className="py-3 px-2 text-right">₹{Number(item.salePrice || 0).toFixed(2)}</td>
          ...
          <div className="mt-8 pt-4 border-t text-center text-sm text-gray-500">
            ...
            <p className="text-xs mt-2 text-gray-400">© {new Date().getFullYear()} POS System. All rights reserved.</p>
          </div>
        </div>
      </div>
                ...
                <div className="mb-4">
                  ...
                  {returnAmount > 0 && (
                    <div className="mt-2 p-2 bg-green-50 border border-green-200 rounded-lg">
                      ...
                      <p className="text-lg font-bold text-green-600">₹{returnAmount.toFixed(2)}</p>
                    </div>
                  )}
                </div>
              )}
      ...
      {showCompleteModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl max-w-md w-full text-center p-8">
            ...
            <p className="text-gray-500 mb-4">Amount: ₹{grandTotal.toFixed(2)}</p>
            <div className="bg-gray-50 rounded-lg p-3 mb-4 text-left">
              <p className="text-sm text-gray-600"><span className="font-medium">Customer:</span> {customerName}</p>
              <p className="text-sm text-gray-600"><span className="font-medium">Payment:</span> {
                selectedPayment === 'cash' ? 'Hand Cash' : 
